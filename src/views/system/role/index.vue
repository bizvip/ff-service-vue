<template>
  <div>
    <a-alert closable>说明：角色是一组权限的集合，可以任命任意账号为任意角色</a-alert>
      ...
      <ma-crud :options="crud" :columns="columns" ref="crudRef">
        ...
        <template #operationBeforeExtend="{ record }">
          <a-space size="mini" v-if="record.code !== 'superAdmin' && ! isRecovery">
            <a-link @click="openMenuList(record)">
              <icon-menu/>
              菜单权限
            </a-link>
            <a-link @click="openDataScopeList(record)">
              <icon-layers/>
              数据权限
            </a-link>
          </a-space>
...
const changeStatus = async (status, record) => {
  ...
  const response = await role.changeStatus({
    id: record.id,
    status
  })
  if (response.success) {
...
const changeSort = async (value, id) => {
  ...
  const response = await role.numberOperation({
    id,
    numberName: 'sort',
    numberValue: value
  })
