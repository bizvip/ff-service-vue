  ...
  <div>
    <a-alert type="info" :show-icon="false">
      <a-space>
        ...
        <icon-notification/>
        <a-typography-text>不同域名展示相同类型的内容可以不一样</a-typography-text>
      </a-space>
    </a-alert>
...
const options = reactive({
  ...
  add: {
    show: true,
    api: staticText.save,
    auth: ['static:text:save']
  },
  edit: {
    show: true,
    api: staticText.update,
    auth: ['static:text:update']
  },
  delete: {
    show: true,
    api: staticText.deletes,
    auth: ['static:text:delete'],
    realApi: staticText.realDeletes,
    realAuth: ['static:text:realDeletes']
  },
  recovery: {
    show: true,
    api: staticText.recoverys,
    auth: ['static:text:recovery']
  },
...
<script> export default { name: 'static:text' } </script>
