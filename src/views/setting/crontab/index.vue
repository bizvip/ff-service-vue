    ...
    <ma-crud :options="crud" :columns="columns" ref="crudRef">
      ...
      <template #operationBeforeExtend="{ record }">
        <a-popconfirm content="确定立刻执行一次?" position="bottom" @ok="run(record)">
          <a-link v-auth="['setting:crontab:run']">
            <icon-caret-right/>
            执行一次
          </a-link>
        </a-popconfirm>
        <a-link @click="openLogModal(record)">
          <icon-history/>
          日志
        </a-link>
      </template>
...
const changeStatus = async (status, id) => {
  const response = await crontab.changeStatus({
    id,
    status
  })
  if (response.success) {
...
const crud = reactive({
  ...
  add: {
    show: true,
    api: crontab.save,
    auth: ['setting:crontab:save']
  },
  edit: {
    show: true,
    api: crontab.update,
    auth: ['setting:crontab:update']
  },
  delete: {
    show: true,
    api: crontab.deletes,
    auth: ['setting:crontab:delete'],
  },
})
...
const columns = reactive([
  ...
  },
  {
    ...
    extra: '例如：30 */5 * * * *，代表每隔5分钟的第30秒执行任务',
    width: 150
  },
  {
    title: '调用目标',
