  ...
  <div>
    <a-alert type="info" :show-icon="false">
      <a-space>
        ...
        >
        <a-typography-text bold underline>底部固定广告</a-typography-text>
        <icon-notification/>
    ...
    <div class="ma-content-block lg:flex justify-between p-4">
      <ma-crud :options="options" :columns="columns" ref="crudRef">
        <template #img_src="{record}">
          <a-image width="200" height="70" fit="contain" :alt="record.title" :src="record.img_src"/>
        </template>
...
const options = reactive({
  ...
  add: {
    ...
    auth: ['caoliu:fixed:save']
  },
  edit: {
    show: true,
    api: staticBanner.update,
    auth: ['caoliu:fixed:update']
  },
  delete: {
    show: true,
    api: staticBanner.deletes,
    auth: ['caoliu:fixed:delete'],
    realApi: staticBanner.realDeletes,
    realAuth: ['caoliu:fixed:realDeletes']
  },
  recovery: {
    show: true,
    api: staticBanner.recoverys,
    auth: ['caoliu:fixed:recovery']
  },
...
<script> export default { name: 'caoliu:fixed' } </script>