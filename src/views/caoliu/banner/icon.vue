  ...
  <div>
    <a-alert type="info" :show-icon="false">
      <a-space>
        ...
        <icon-notification/>
        <a-typography-text>由于CDN等缓存分发的作用，时间设置可能存在数分钟延迟</a-typography-text>
      </a-space>
    </a-alert>
    <div class="ma-content-block lg:flex justify-between p-4">
      <ma-crud :options="options" :columns="columns" ref="crudRef">
        <template #img_src="{record}">
          <a-image width="100" height="100" fit="contain" :alt="record.title" :src="record.img_src"/>
        </template>
...
const options = reactive({
  ...
  add: {
    ...
    auth: ['caoliu:icon:save']
  },
  edit: {
    show: true,
    api: staticBanner.update,
    auth: ['caoliu:icon:update']
  },
  delete: {
    show: true,
    api: staticBanner.deletes,
    auth: ['caoliu:icon:delete'],
    realApi: staticBanner.realDeletes,
    realAuth: ['caoliu:icon:realDeletes']
  },
  recovery: {
    show: true,
    api: staticBanner.recoverys,
    auth: ['caoliu:icon:recovery']
  },
...
<script> export default { name: 'caoliu:icon' } </script>